...
export class InoutnetworkChartComponent   {
 ...
 reportData = inject(ReportService)['reportDataSubject'].value;
  ...
  get inPercentage(): any {
    return this.total ? (this.inNetworkTotal / this.total *100 ) .toFixed(2) + '%' : '0%';
  }

  get outPercentage(): string {
    return this.total ? (this.outNetworkTotal / this.total * 100 ) .toFixed(2) + '%' : '0%';
  }
